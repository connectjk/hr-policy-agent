...
def deploy():
    ...
    payload = json.dumps({
        "name": sanitize_name("HR-Policy--Agent"),
        "description": """A HR Policy support agent that can search our knowledge base, Provide a HR Policy Support related information for banking customer""",
        "definition": {
            "kind": "prompt",
            "model": "gpt-4o",
            "instructions": """A HR Policy support agent that can search our knowledge base, Provide a HR Policy Support related information for banking customer"""
        }
    }).encode()
